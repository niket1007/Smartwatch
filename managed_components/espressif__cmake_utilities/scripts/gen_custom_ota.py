...
def xz_compress(store_directory, in_file):
    ...
    with open(in_file, "rb") as src_f:
        ...
        with lzma.open(
            ...
            filters=xz_compressor_filter,
        ) as f:
            f.write(data)
            f.close()

    if not os.path.exists(
        os.path.join(store_directory, os.path.split(compressed_file)[1])
    ):
...
def secure_boot_sign(sign_key, in_file, out_file):
    ret = subprocess.call(
        "espsecure.py sign_data --version 2 --keyfile {} --output {} {}".format(
            sign_key, out_file, in_file
        ),
        shell=True,
    )
    if ret:
...
def main():
    ...
    parser.add_argument(
        ...
        choices=["v1", "v2", "v3"],
        default="v3",
        help="the version of the packed file header [default:v3]",
    )
    parser.add_argument(
        "-c",
        "--compress_type",
        ...
        choices=["none", "xz"],
        default="xz",
        help="compressed type [default:xz]",
    )
    parser.add_argument(
        "-i", "--in_file", nargs="?", default="", help="the new app firmware"
    )
    parser.add_argument(
        "--sign_key", nargs="?", default="", help="the sign key used for secure boot"
    )
    parser.add_argument(
        "-fv",
        "--fw_ver",
        nargs="?",
        default="",
        help="the version of the compressed data(this field is deprecated in v3)",
    )
    parser.add_argument(
        "--add_app_header",
        action="store_true",
        help="add app header to use native esp_ota_* & esp_https_ota_* APIs",
    )
    parser.add_argument(
        "-v",
        "--version",
        ...
        help="the version of the script",
    )
