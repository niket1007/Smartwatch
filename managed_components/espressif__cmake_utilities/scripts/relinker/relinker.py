...
def lib_secs(lib, file, lib_path):
    ...
    new_env["LC_ALL"] = "C"
    dump = StringIO(
        subprocess.check_output([espidf_objdump, "-h", lib_path], env=new_env).decode()
    )
    dump.name = lib
...
class relink_c:
    def __init__(
        ...
        for i in libraries.libs:
            ...
            for j in lib.objs:
                obj = lib.objs[j]
                self.targets.append(
                    target_c(lib.name, lib.path, obj.name, " ".join(obj.sections()))
                )
    ...
    def __transform__(self):
        ...
        self.iram1_exclude = (
            "    *(EXCLUDE_FILE(%s %s) .iram1.*) *(EXCLUDE_FILE(%s %s) .iram1)"
            % (
                self.filter.add(),
                " ".join(iram1_exclude),
                self.filter.add(),
    ...
    def __replace__(self, lines):
        def is_iram_desc(l):
            if "*(.iram1 .iram1.*)" in l or (
                ") .iram1 EXCLUDE_FILE(*" in l and ") .iram1.*)" in l
            ):
                return True
            return False
                ...
                if index in l and t.file.split(".")[0] not in l:
                    for m in self.targets:
                        ...
                        if index in l and m.file.split(".")[0] not in l:
                            l = l.replace(
                                "EXCLUDE_FILE(", "EXCLUDE_FILE(%s " % (m.desc)
                            )
                            if len(m.isecs) > 0:
...
def main():
    ...
    argparser.add_argument("--input", "-i", help="Linker template file", type=str)

    argparser.add_argument("--output", "-o", help="Output linker script", type=str)

    argparser.add_argument(
        "--library", "-l", help="Library description directory", type=str
    )

    argparser.add_argument("--object", "-b", help="Object description file", type=str)

    argparser.add_argument(
        "--function", "-f", help="Function description file", type=str
    )

    argparser.add_argument("--sdkconfig", "-s", help="sdkconfig file", type=str)

    argparser.add_argument("--objdump", "-g", help="GCC objdump command", type=str)

    argparser.add_argument(
        "--debug", "-d", help="Debug level(option is 'debug')", default="no", type=str
    )
