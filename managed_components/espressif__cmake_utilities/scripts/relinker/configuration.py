            ...
            try:
                dump = StringIO(
                    subprocess.check_output(
                        [espidf_objdump, "-t", path], env=new_env
                    ).decode()
                )
...
def generator(
    ...
    for d in csv.DictReader(open(function_file, "r")):
        ...
        if not obj_path:
            obj_path = lib_path
        libraries.append(
            d["library"], lib_path[0], d["object"], obj_path, d["function"]
        )
    return libraries
...
def main():
    ...
    argparser.add_argument("--library", "-l", help="Library description file", type=str)
