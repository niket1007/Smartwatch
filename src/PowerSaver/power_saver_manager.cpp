...
esp_err_t PowerSaverManager::init()
{
    ...
    {
        usb_serial.println("Failed to create brightness mutex");
        return ESP_ERR_NO_MEM;
    }

    int batt = battery_manager.get_battery_percentage();
    change_screen_timeout_value(batt);
    change_brightness_value(batt);
...
esp_err_t PowerSaverManager::change_brightness_value(int batt)
{
    ...
    display_manager.set_brightness(current_brightness_percentage_);
...
bool PowerSaverManager::is_touch_disabled()
{
    return is_touch_disabled_;
}

esp_err_t PowerSaverManager::handle_ble_event(uint32_t events)
{
    if (events & BLUETOOTH_DEINIT_EVENT)
    {
        if(bluetooth_manager.is_init())
        {
            esp_err_t err = bluetooth_manager.deinit();
            if(err != ESP_OK)
            {
                usb_serial.println("Failed to deinit bluetooth");
            }
        }
    }
    else if (events & BLUETOOTH_INIT_EVENT)
    {
        if(!bluetooth_manager.is_init())
        {
            ...
            if(err != ESP_OK)
            {
                usb_serial.println("Failed to init bluetooth");
            }
        }
    }
...
{
    if ((events & BAT_PERCENT_CHG_EVENT) and !display_manager.is_sleeping())
    {
        int batt = battery_manager.get_battery_percentage();

        esp_err_t ret = change_brightness_value(batt);
        if(ret != ESP_OK)
        {
            usb_serial.println("Failed to update brightness");
        }

        ret = change_screen_timeout_value(batt);
        if(ret != ESP_OK)
        {
            usb_serial.println("Failed to update screen timeout");
        }
    }

    if(events & TOUCH_DISABLED_EVENT)
    {
        is_touch_disabled_ = true;
    }

    if(events & TOUCH_ENABLED_EVENT)
    {
        is_touch_disabled_ = false;
    }
